...
db.prototype = {
	...
	getAllRooms: function(callback) {
		redisClient.smembers(REDIS_PREFIX + '-rooms', function (err, res) {
			callback(res);
		});
	},
	...
	createCard: function(room, id, card) {
		...
		redisClient.sadd(REDIS_PREFIX + '-rooms', room);
	},
	...
	deleteCard: function(room, id) {
		...
		redisClient.hlen(REDIS_PREFIX + '-room:' + room + '-cards', function (err, res) {
			if (res) {
				redisClient.srem(REDIS_PREFIX + '-rooms', room);
			}
		});
	},
